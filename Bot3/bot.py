...
class ChatBot:
    ...
    async def _setup_page(self, page: Page) -> None:
        ...
        elif name == "StrangerMeetup":
            await self._setup_strangermeetup(page)
        else:
            self._ctx = page

    async def _setup_strangermeetup(self, page: Page) -> None:
        """StrangerMeetup: click Start the chat to confirm age and enter."""
        self._ctx = page
        try:
            await page.click("text=Start the chat", timeout=8000)
            log(self.name, "StrangerMeetup setup done — waiting for stranger")
        except Exception as e:
            log(self.name, f"StrangerMeetup setup error: {e}")
